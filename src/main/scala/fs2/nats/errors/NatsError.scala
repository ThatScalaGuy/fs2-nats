...
object NatsError:
  ...
  /**
   * Payload exceeds the server's max_payload limit.
   * Check Info.maxPayload and ensure messages are within bounds.
   *
   * @param size The attempted payload size in bytes
   * @param maxPayload The server's maximum allowed payload size
   */
  ...
  /**
   * Error parsing NATS protocol data from the server.
   * May indicate protocol version mismatch or corrupted data.
   *
   * @param description Description of what failed to parse
   * @param cause Optional underlying parsing exception
   */
  ...
  /**
   * Failed to establish or maintain connection to NATS server.
   * May occur on initial connect or during reconnection attempts.
   *
   * @param description Description of the connection failure
   * @param cause Optional underlying network exception
   */
  ...
  /**
   * Subscription queue is full and slow consumer policy triggered.
   * Either increase queue capacity or improve consumer throughput.
   *
   * @param sid The subscription ID experiencing slow consumption
  ...
  case object ClientClosed
      extends NatsError("Client has been closed")

  /**
   * TLS handshake or configuration error.
   *
   * @param description Description of the TLS error
   * @param cause Optional underlying TLS exception
   */
   ...
   * Subjects cannot contain spaces or certain special characters.
   *
   * @param subject The invalid subject string
   * @param reason Why the subject is invalid
   */
   ...
   * The server sent an -ERR response.
   *
   * @param serverMessage The error message from the server
   */
