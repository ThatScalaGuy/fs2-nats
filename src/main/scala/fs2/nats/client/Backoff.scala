...
/** Backoff policy for calculating delays between retry attempts. Immutable and
  * thread-safe.
  */
trait BackoffPolicy:
  ...
  /** Calculate the delay for a given retry attempt.
    ...
    * @return
    *   The delay before this attempt, or None if max retries exceeded
    */
...
object Backoff:
  ...
  /** Create an exponential backoff policy with full jitter.
    ...
    * calculated delay, which helps decorrelate retry storms in distributed
    * systems.
    *
    * Delay calculation: min(maxDelay, baseDelay * factor^(attempt-1)) *
    * random(0, 1)
    *
    * @param base
    ...
    * @return
    *   A BackoffPolicy implementing exponential backoff with full jitter
    */
      ...
      maxRetries match
        ...
        case _                                          =>
          val exponentialDelay =
            base.toMillis.toDouble * math.pow(factor, (attempt - 1).toDouble)
  ...
  /** Create an exponential backoff policy with decorrelated jitter.
    *
    * Decorrelated jitter provides slightly better properties for load
    * distribution than full jitter by making each delay partially dependent on
    * the previous.
    *
    * @param base
    *   The initial delay before first retry
    ...
    * @return
    *   A BackoffPolicy implementing decorrelated jitter
    */
        ...
        case _                                          =>
          if attempt == 1 then
            ...
            val nextDelay = math.min(
              max.toMillis,
              random.between(base.toMillis, lastDelay * 3)
            )
            lastDelay = nextDelay
  ...
  /** Create a fixed delay backoff policy.
    ...
    *   Optional maximum number of retry attempts
    * @return
    *   A BackoffPolicy with constant delay
    */
  def fixed(
  ...
  /** Create a backoff policy with no delay.
    ...
    * @return
    *   A BackoffPolicy with zero delay
    */
  ...
  /** Create a backoff policy from a BackoffConfig.
    ...
    * @return
    *   A BackoffPolicy based on the config
    */
...
object Retry:
  ...
  /** Retry an effect using a backoff policy.
    ...
    * @param onRetry
    *   Callback invoked before each retry with (attempt, delay, error)
    * @tparam F
    *   The effect type
    * @tparam A
    *   The result type
    * @return
    *   The effect wrapped with retry logic
    */
  def withBackoff[F[_]: Async, A](
    ...
    def loop(attempt: Int): F[A] =
      fa.handleErrorWith { error =>
        if !shouldRetry(error) then Async[F].raiseError(error)
        else
