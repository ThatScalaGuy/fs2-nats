...
/**
 * NATS message headers following the NATS/1.0 format.
 * Headers are key-value pairs where keys are case-insensitive for lookup
 * but preserved for serialization. Multiple values per key are supported.
 *
 * Format:
 * {{{
 * NATS/1.0\r\n
 ...
 * }}}
 *
 * @param entries The header entries as a Vector of key-value pairs
 */
final case class Headers(entries: Vector[(String, String)]):
   ...
   * @return The first value if found
   */
   ...
   * @return New Headers with the added entry
   */
   ...
   * @param key The header key
   * @param value The header value
   * @return New Headers with the key set to the single value
   */
   ...
   * @return New Headers without entries for the key
   */
   ...
   *
   * @return Chunk of bytes in NATS header format
   */
   ...
   *
   * @return The byte length when serialized
   */
  def byteLength: Int =
...
object Headers:
   ...
   * @return Headers containing the pairs
   */
   ...
   *
   * @param map The map of header keys to values
   * @return Headers containing the map entries
   */
   ...
   *
   * @param bytes The header bytes to parse
   * @return Either a parse error or the parsed Headers
   */
  ...
  def parse(str: String): Either[String, Headers] =
    ...
    if lines.isEmpty || !lines.head.startsWith(Version) then
      ...
      val parsed = headerLines.map { line =>
        ...
        if colonIdx < 0 then
          Left(s"Invalid header line (no colon): $line")
        else
  ...
  /**
   * Parse headers from NATS/1.0 format bytes, extracting status code if present.
   * Status codes appear as "NATS/1.0 503" for no-responders, etc.
   *
   * @param bytes The header bytes to parse
   * @return Either a parse error or tuple of (optional status code, Headers)
   */
  def parseWithStatus(bytes: Chunk[Byte]): Either[String, (Option[Int], Headers)] =
    val str = new String(bytes.toArray, StandardCharsets.UTF_8)
    parseWithStatus(str)

  /**
   * Parse headers from NATS/1.0 format string, extracting status code if present.
   *
   * @param str The header string to parse
   * @return Either a parse error or tuple of (optional status code, Headers)
   */
  def parseWithStatus(str: String): Either[String, (Option[Int], Headers)] =
    ...
    if lines.isEmpty then
      Left("Invalid NATS headers: empty input")
    else
      ...
      if !versionLine.startsWith(Version) then
        ...
        val errors = parsed.collect { case Left(e) => e }
        if errors.nonEmpty then Left(errors.mkString("; "))
        else Right((statusCode, Headers(parsed.collect { case Right(kv) => kv })))
