...
/**
 * Server INFO message sent upon connection and asynchronously when cluster topology changes.
 * Contains server metadata and connection parameters.
 *
 * @param serverId The unique identifier of the server
 * @param serverName The name of the server
 * @param version The version string of the NATS server
 * @param proto Protocol version in use
 * @param go Go version used to build the server
 * @param host The host this server is bound to
 * @param port The port this server is bound to
 * @param headersSupported Whether the server supports headers (NATS 2.2+)
 * @param maxPayload Maximum payload size the server will accept
 ...
 * @param tlsVerify Whether TLS client certificate verification is required
 * @param tlsAvailable Whether TLS is available
 * @param connectUrls List of server URLs for cluster-aware clients
 * @param ldmMode Whether the server is in Lame Duck Mode
 * @param jetStream Whether JetStream is enabled (not used in this iteration)
 * @param nonce Authentication nonce for NKey authentication
 */
...
object Info:
  given Decoder[Info] = Decoder.instance { c =>
    for
      ...
      headersSupported <- c.downField("headers").as[Option[Boolean]].map(_.getOrElse(false))
      maxPayload <- c.downField("max_payload").as[Long]
      clientId <- c.downField("client_id").as[Option[Long]]
      authRequired <- c.downField("auth_required").as[Option[Boolean]].map(_.getOrElse(false))
      tlsRequired <- c.downField("tls_required").as[Option[Boolean]].map(_.getOrElse(false))
      tlsVerify <- c.downField("tls_verify").as[Option[Boolean]].map(_.getOrElse(false))
      tlsAvailable <- c.downField("tls_available").as[Option[Boolean]].map(_.getOrElse(false))
      ...
      jetStream <- c.downField("jetstream").as[Option[Boolean]].map(_.getOrElse(false))
...
/**
 * Client CONNECT message sent after receiving INFO from server.
 * Contains client metadata and authentication credentials.
 *
 * @param verbose If true, server sends +OK for each correctly parsed command
 * @param pedantic If true, server performs stricter checking
 * @param tlsRequired If true, client requires TLS connection
 * @param authToken Authentication token if required
 * @param user Username for user/password authentication
 * @param pass Password for user/password authentication
 * @param name Optional client name for identification
 * @param lang Client implementation language
 * @param version Client version
 * @param protocol Protocol version
 * @param echo If false, server won't echo messages published by this client
 * @param sig Signature for NKey authentication
 * @param jwt User JWT for JWT authentication
 * @param nkey Public NKey for NKey authentication
 * @param headersSupported Whether client supports headers
 * @param noResponders Whether to enable no-responders behavior
 */
 ...
 * @param subject The subject to publish to
 * @param replyTo Optional reply-to subject for request/reply pattern
 * @param headerLength The length of the headers block in bytes
 * @param totalLength The total length of headers + payload in bytes
 */
 ...
 *
 * @param subject The subject to subscribe to (may include wildcards)
 * @param queueGroup Optional queue group for load balancing
 * @param sid The subscription ID (unique per connection)
 */
 ...
 *
 * @param sid The subscription ID to unsubscribe
 * @param maxMsgs Optional max messages to receive before auto-unsubscribing
 */
 ...
 * @param sid The subscription ID that matched
 * @param replyTo Optional reply-to subject
 * @param payloadLength The length of the payload in bytes
 */
