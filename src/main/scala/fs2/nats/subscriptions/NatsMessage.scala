...
/** Represents a message received from a NATS subscription. This is the
  ...
  * @param headers
  *   The message headers (empty if no headers)
  * @param payload
...
object NatsMessage:
  /** Create a message with just a subject and payload (no headers).
    ...
    *   The subscription ID
    * @return
    *   A NatsMessage without headers or reply-to
    */
  ...
  /** Create a message from a UTF-8 string payload.
    ...
    *   The subscription ID
    * @return
    *   A NatsMessage with the string encoded as UTF-8
    */
