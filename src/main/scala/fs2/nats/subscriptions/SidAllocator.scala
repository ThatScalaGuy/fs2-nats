...
/** Allocator for subscription IDs (SIDs).
  *
  * SIDs are unique identifiers for subscriptions within a NATS connection. This
  * allocator provides monotonically increasing IDs and tracks which IDs are
  * currently in use for potential reuse in long-running connections.
  *
  * @tparam F
  *   The effect type
  */
trait SidAllocator[F[_]]:
  ...
  /** Allocate the next available subscription ID.
    *
    * @return
    *   The next unique subscription ID
    */
  def next: F[Long]

  /** Release a subscription ID back to the pool. Currently, IDs are not reused
    * but this allows for future optimization.
    *
    * @param sid
    *   The subscription ID to release
    * @return
    *   Effect that completes when the ID is released
    */
  ...
  /** Get all active subscription IDs.
    *
    * @return
    *   Set of all active subscription IDs
    */
