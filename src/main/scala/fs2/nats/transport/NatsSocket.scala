...
/** NATS socket transport implementation.
  *
  * Creates a transport over a TCP socket connection to a NATS server. Frames
  * are parsed using the incremental protocol parser, and writes are serialized
  * through a bounded queue processed by a single writer fiber.
  */
object NatsSocket:
  ...
  def resource[F[_]: Async: Network](
    ...
    for
      socket <- Network[F].connect(address, config.socketOptions)
